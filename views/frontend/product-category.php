         ...
         <div class="row">
            ...
            <div class="col-md-9 order-1 order-md-2">
               ...
               <div class="product-category mt-3">
                  <div class="text-end pb-4">
                     ...
                     <select name="sort" id="sort" onchange="this.options[this.selectedIndex].value && (window.location=this.options[this.selectedIndex].value)" >
                        <option value="index.php?option=product&cat=<?=$slug;?><?php if(isset($_REQUEST['page'])) echo "&page=".$_REQUEST['page'] ;?>" >Lọc</option>
                        <option value="index.php?option=product&cat=<?=$slug;?>&sort=ASC<?php if(isset($_REQUEST['page'])) echo "&page=".$_REQUEST['page'] ;?>" 
                        <?=isset($_REQUEST['sort'])&&$_REQUEST['sort']=='ASC'?'selected':"";?>>Giá tăng dần</option>
                        <option value="index.php?option=product&cat=<?=$slug;?>&sort=DESC<?php if(isset($_REQUEST['page'])) echo "&page=".$_REQUEST['page'] ;?>" 
                        <?=isset($_REQUEST['sort'])&&$_REQUEST['sort']=='DESC'?'selected':"";?>>Giá giảm dần</option>
                        <option value="index.php?option=product&cat=<?=$slug;?>&sort=moi<?php if(isset($_REQUEST['page'])) echo "&page=".$_REQUEST['page'] ;?>" 
                        <?=isset($_REQUEST['sort'])&&$_REQUEST['sort']=='moi'?'selected':"";?>>Mới -> Cũ</option>
                        <option value="index.php?option=product&cat=<?=$slug;?>&sort=cu<?php if(isset($_REQUEST['page'])) echo "&page=".$_REQUEST['page'] ;?>" 
               ...
               <?php 
                  if(isset($_REQUEST['sort']))
                  {
                     $url='index.php?option=product&cat='.$slug.'&sort='.$_REQUEST['sort'];
                  }
                  else
                  {
                     $url='index.php?option=product&cat='.$slug;
                  }
               ?>
