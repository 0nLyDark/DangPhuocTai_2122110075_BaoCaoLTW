<div class="product-item border">
    ...
    <h3 class="product-item-price fs-6 p-2 d-flex ">
        <?php if($product->pricesale != $product->price):?>
            <div class="flex-fill text-secondary"><del><?=number_format($product->pricesale);?>đ</del></div>
            <div class="flex-fill text-end text-main"><?=number_format($product->price);?>đ</div>
        <?php else:?>
            <div class="flex-fill text-center text-main"><?=number_format($product->price);?>đ</div>
        <?php endif;?>
    </h3>
</div>